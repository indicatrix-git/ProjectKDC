...
namespace Ditec.RIS.RFO.View
{
	...
	[Implements(typeof(DetailView), ServiceName = ModuleScenes.RFODetail)]
	public partial class RFODetailView : DetailView
	{
		public RFODetailView()
		{
			Localizer.Active = new MyLocalizer();
			InitializeComponent();
		}

		public RFODetailView(IShell shell)
			: base(shell)
		{
			Localizer.Active = new MyLocalizer();
			InitializeComponent();
		...
		private void InitializeUI()
		{
			...
			ViewHelper.SetReadOnlyItemsInGroupExceptSpecifiedControls(this.lcgRFOIdentifikacneUdaje, true, new Control[] { this.gcStatnaPrislusnost, this.gcObmPozbavPravnejSposobilosti, this.btnZatvorit });
		...
		private void RFODetailView_Shown(object sender, EventArgs e)
		{
			...
			if ((this.Model as RFODetailModel).OsobaZRfo != null)
			{
				(this.Model as RFODetailModel).Data = (this.Model as RFODetailModel).OsobaZRfo;
			}
		}
		...
		protected override void OnDataChanged()
		{
			...
			this.model = this.Model as RFODetailModel;
